...
- **Legacy `herdr` migration.** The previous `<git-common-dir>/herdr` root is retained as a compatibility source only: it is never auto-deleted and never acts as a second canonical authority. Before every plan or execution read/write the service reconciles the legacy root into the canonical Flocky root — legacy-only artifacts are validated, staged, and atomically promoted (preserving valid legacy artifacts such as plan ID `developer-steering-flocky-state-20260901-01`); byte-identical copies are accepted; divergent valid copies fail closed with a structured `MIGRATION_CONFLICT` (no silent selection or replacement, so an active legacy write is surfaced instead of lost to last-writer-wins). A migration lock (a generation-carrying owner-token lease: takeover is a compare-and-swap on the observed lease bytes — a contender may only displace the exact stale lease it observed — and release is ownership-verified) and a write-ahead journal make reconciliation recoverable: a later service call resumes an interrupted promotion or rolls it back.
